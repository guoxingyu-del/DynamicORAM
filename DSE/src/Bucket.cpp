...
Block Bucket::GetBlockByIndex(array<int8_t, Block::KEY_SIZE> keyword) {
...
bool Bucket::RemoveBlock(const Block& rm_blk){
    bool removed = false;
    for(int i = 0; i < blocks.size(); i++){
        if(blocks[i].keyword == rm_blk.keyword){
            blocks[i] = Block();
            removed = true;
            break;
        }
...
bool Bucket::HasEmpty() {
    for (int i = 0; i < blocks.size(); i++) {
        if (blocks[i].document_id == -1) return true;
    }

...
Block ServerStorage::ReadSubtreeMax(int position) {
    if (!this->is_capacity_set) {
		throw new runtime_error("Please call setCapacity before reading or writing any block");
	}
    ...
    return this->buckets[position].GetSubTreemax();
}
    
void ServerStorage::WriteSubtreeMax(int position, const Block& subtree_max) {
    if (!this->is_capacity_set) {
		throw new runtime_error("Please call setCapacity before reading or writing any block");
	}
    ...
    this->buckets[position].SetSubTreemax(subtree_max);
}